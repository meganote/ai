...
def group_tool_calls(chat_ctx: bot.ChatContext) -> list[_ChatItemGroup]:
    ...
    tool_outputs: list[bot.FunctionCallOutput] = []
...
class _ChatItemGroup:
    message: bot.ChatMessage | None = None
    tool_calls: list[bot.FunctionCall] = field(default_factory=list)
    tool_outputs: list[bot.FunctionCallOutput] = field(default_factory=list)

    def add(self, item: bot.ChatItem) -> _ChatItemGroup:
    ...
    def flatten(self) -> list[bot.ChatItem]:
        items: list[bot.ChatItem] = []
        if self.message:
